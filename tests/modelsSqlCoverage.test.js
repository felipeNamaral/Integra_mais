...
test('User model busca usuarios e controla token de recuperacao', async (t) => {
  ...
  await User.salvarResetTokenUsuario('u@teste.com', 'token');
  await User.salvarResetTokenEmpresa('e@teste.com', 'token-e');
  ...
  assert.deepEqual(chamadas[2][1], ['token', 'token']);
  assert.match(chamadas[3][0], /UPDATE usuario SET reset_token = \?, reset_token_expira = DATE_ADD\(SYSDATE\(\), INTERVAL 15 MINUTE\) WHERE email = \?/);
  assert.equal(chamadas[3][1][0], 'token');
  assert.equal(chamadas[3][1][1], 'u@teste.com');
  assert.match(chamadas[4][0], /UPDATE empresa SET reset_token = \?, reset_token_expira = DATE_ADD\(SYSDATE\(\), INTERVAL 15 MINUTE\) WHERE email = \?/);
  assert.equal(chamadas[4][1][0], 'token-e');
  assert.equal(chamadas[4][1][1], 'e@teste.com');
  assert.match(chamadas[5][0], /UPDATE usuario SET senha = \?, reset_token = NULL, reset_token_expira = NULL WHERE email = \?/);
  assert.deepEqual(chamadas[5][1], ['hash-u', 'u@teste.com']);
  assert.match(chamadas[6][0], /UPDATE empresa SET senha = \?, reset_token = NULL, reset_token_expira = NULL WHERE email = \?/);
  assert.deepEqual(chamadas[6][1], ['hash-e', 'e@teste.com']);
});
